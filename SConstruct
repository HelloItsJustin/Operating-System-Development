...
# === HOST ENVIRONMENT ===
HOST_ENVIRONMENT = Environment(variables=VARS, ENV=os.environ)

platform_prefix = ''
if HOST_ENVIRONMENT['arch'] == 'i686':
    platform_prefix = 'i686-elf-'

# Set NASM as assembler
...
# === TOOLCHAIN PATH FIX ===
# Hardcoded to: toolchain/i686-elf
toolchainDir = Path('toolchain/i686-elf').resolve()
toolchainBin = Path(toolchainDir, 'bin')
toolchainGccLibs = Path(toolchainDir, 'lib', 'gcc', 'i686-elf', DEPS['gcc'])
